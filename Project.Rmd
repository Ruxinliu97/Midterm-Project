...
### Model Selection & Validation 
  ...
  negative binomial model is confirmed to be the appropriate model for this data 
  ...
  fitted with the *lme4* package in R, where the delayed time is the response, 
...
# Result
  ...
  From Fig.4, the effects and their 95% confidence intervals of the multilevel model 
  are shown, where the effects are calculated by taking the exponential of the estimated 
  coefficients. 
...
```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.cap = "From this plot, there is no obvious difference between the 12 months, therefore the month is not used for grouping in the model."}
month <- delay_final %>% 
  filter(Min.Delay < 60) %>% 
...
par(mfrow=c(2,2))
# Assumptions are violated, suggesting that linear model is not appropriate. 
...
```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.cap = ""}
...
```{r, message = FALSE}
# Final multilevel negative binomial regression model
library(lme4)
nb_route <- glmer.nb(Min.Delay ~ Incident + weekend + (1 | Route), data = delay_final)
```


```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.cap = "Although there are still some overdispersion at the lower counts, the residuals overall look fine. "}
